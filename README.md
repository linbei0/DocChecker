...
- 发布规则集时可保存为个人模板，记录学校、学院、论文类型、版本、规则数量和版本历史。
- 检查结果包含严重程度、规则来源、期望值、实际值、定位信息、上下文片段和修改建议。
- 生成 Markdown 报告，并持久化任务、规则集和报告状态。
- 支持默认内联后台任务，也可切换到 RQ + Redis worker 执行检查和规范文档规则抽取。
...
默认 `DOC_CHECKER_TASK_EXECUTION_MODE=inline` 不需要 Redis。若切换为 `rq`，先启动 Redis，再启动 worker：

```powershell
$env:DOC_CHECKER_TASK_EXECUTION_MODE="rq"
$env:DOC_CHECKER_REDIS_URL="redis://localhost:6379/0"
uv run docchecker-worker
```

API 进程在 `rq` 模式下只创建任务、写入初始状态并入队；检查任务和规范文档规则抽取由 worker 执行。RQ job 会使用配置的超时、重试间隔、结果 TTL 和失败 TTL；Redis 不可用时接口会返回明确的 503 错误并把已创建的任务标记为失败。
...
| `DOC_CHECKER_TASK_EXECUTION_MODE` | `inline` | 任务执行模式，支持 `inline` 或 `rq` |
| `DOC_CHECKER_REDIS_URL` | `redis://localhost:6379/0` | RQ 模式使用的 Redis 地址 |
| `DOC_CHECKER_RQ_QUEUE_NAME` | `docchecker` | RQ 队列名称 |
| `DOC_CHECKER_RQ_JOB_TIMEOUT_SECONDS` | `900` | RQ job 执行超时秒数 |
| `DOC_CHECKER_RQ_RESULT_TTL_SECONDS` | `86400` | RQ 成功结果保留秒数 |
| `DOC_CHECKER_RQ_FAILURE_TTL_SECONDS` | `604800` | RQ 失败结果保留秒数 |
| `DOC_CHECKER_RQ_RETRY_MAX` | `3` | RQ 可重试异常最大重试次数 |
| `DOC_CHECKER_RQ_RETRY_INTERVALS_SECONDS` | `[10,60,300]` | RQ 重试间隔，`.env` 中按 JSON 数组填写 |
...
| `GET` | `/api/rulesets` | 获取最新未归档规则模板列表 |
| `PATCH` | `/api/rulesets/{ruleset_id}` | 更新模板元数据并生成新版本 |
| `GET` | `/api/rulesets/{ruleset_id}/versions` | 获取同一模板的版本历史 |
...
3. 在前端确认、编辑或禁用候选规则。
4. 填写模板名称、学校、学院、论文类型和版本备注，发布为个人模板。
5. 创建检查任务并等待执行完成。
...
- 默认任务执行模式为 `inline`；`rq` 模式需要独立 Redis 服务和 `docchecker-worker` 进程。
